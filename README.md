...
> 一個為 Paper / Spigot / Folia 設計的 Minecraft 養殖插件，把原版「餵食即繁殖」替換為「關係經營 + 全局成長」雙線系統。

## 簡介

BreedingLog 讓養殖變得更有深度：

- 好感度線（個體）：每隻動物都擁有獨立心情值，透過摸摸提升，攻擊或頻繁收穫會降低好感度。好感度決定是否能繁殖。
- 牧場等級線（全局）：玩家透過收穫或擊殺累積經驗，等級會影響掉落表與解鎖進度。
- 性別線（個體）：每隻動物都有性別（公／母），繁殖必須是同種異性且符合條件。
- 領養系統：野外無主動物可以被領養，成為牧場成員。
- 繁殖蛋：孕期結束後，母體會產下蛋，拾起後右鍵即可生成幼崽。
- CraftEngine 整合：特殊掉落物與繁殖蛋使用 CE 自訂物品，支援自訂材質與資源包。
- 多語言支援：繁體中文、英文官方支援，簡體中文由社群貢獻。
...
| 伺服器 | Paper 26.2 / Spigot 26.2 / Folia |
 | Java | 25+（目前 Gradle toolchain 使用 Java 25） |
| 前置插件 | [CraftEngine](https://github.com/Xiao-MoMi/CraftEngine) 26.x（必需） |
| 可選 | SQLite（長期數據備份） |
...
| 1 | 基礎設施（SchedulerAdapter、資料層、事件入口） | 已完成 |
| 2 | 好感度 + 性別線 | 已完成核心流程 |
| 3 | 牧場等級線 | 已完成核心流程 |
| 4 | 繁殖與育幼 | 已完成基礎流程，繁殖蛋待完成 |
| 5 | 指令與優化 | 已完成基礎流程 |
...
進入遊戲，對野生動物空手 + Shift + 右鍵，應該會看到領養提示。
...
1. 複製 `src/main/resources/lang/en_US.yml`
2. 翻譯為你的語言，存為 `lang/xx_XX.yml`
3. 提交 PR
...
- [CraftEngine](https://github.com/Xiao-MoMi/CraftEngine) — 自訂物品與方塊框架
- [Paper](https://papermc.io/) — 高效能伺服器核心
- [Folia](https://papermc.io/software/folia) — 分區多線程伺服器核心
- [Adventure](https://docs.advntr.dev/) — 文本組件 API

---

## 連結

- [完整設計計劃書](docs/DESIGN.md)
- [Issues](https://github.com/yourname/BreedingLog/issues)
- [Discussions](https://github.com/yourname/BreedingLog/discussions)
